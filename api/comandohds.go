...
func (i *Indexer) HandlerComandoHDsIndexer(w http.ResponseWriter, r *http.Request) {
	...
	indexedTorrents := utils.ParallelFlatMap(links, func(link string) ([]schema.IndexedTorrent, error) {
		return getTorrentsComandoHDs(ctx, i, link, url)
	})
...
func getTorrentsComandoHDs(ctx context.Context, i *Indexer, link, referer string) ([]schema.IndexedTorrent, error) {
	...
	for it, magnetLink := range magnetLinks {
		...
		go func(it int, magnetLink string) {
			...
			if err != nil {
				logging.Error().Err(err).Str("magnet_link", magnetLink).Msg("Failed to parse magnet link")
			}
			...
			if err != nil {
				logging.Error().Err(err).Str("info_hash", infoHash).Msg("Failed to get peers and seeds")
			}
